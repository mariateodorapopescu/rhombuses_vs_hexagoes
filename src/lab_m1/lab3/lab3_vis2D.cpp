...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
	...
	{
		logicSpace.width += logicSpace.x;
		logicSpace.height += logicSpace.y;

		logicSpace.width += 0.5f * deltaTime; 
		logicSpace.height += 0.5f * deltaTime;
	...
	if (window->KeyHold(GLFW_KEY_X))
	{
		...
		logicSpace.width -= 5.0f * deltaTime; 
		logicSpace.height -= 5.0f * deltaTime;
	}

    // again, habar nu am ce fac
...
{
    // eu stiam ca e cu on key press, bad cum modific pentru asta
}
